...
void test_ckks() {
    ...
    if (true) {
        ...
        Plaintext plain1; plain1 = "1x^2 + 1";
        Plaintext plain2; plain2 = "1x^2 + 1x^1";
        Plaintext plain3; plain3 = "1x^2 + 1x^1 + 1";
        Ciphertext cipher1; encryptor.encrypt(plain1, cipher1);
        Ciphertext cipher2; encryptor.encrypt(plain2, cipher2);
        Ciphertext cipher3; encryptor.encrypt(plain3, cipher3);
        CiphertextCuda c_cipher1(cipher1);
        CiphertextCuda c_cipher2(cipher2);
        CiphertextCuda c_cipher3(cipher3);
        vector<Ciphertext> ciphers { cipher1, cipher2, cipher3 };
        vector<CiphertextCuda> c_ciphers { c_cipher1, c_cipher2, c_cipher3 };

        Ciphertext product; 
        CiphertextCuda c_product;

        // evaluator.multiply(cipher2, cipher3, product);
        // c_evaluator.multiply(c_cipher2, c_cipher3, c_product);

        // evaluator.relinearizeInplace(product, rlkcpu);
        // c_evaluator.relinearizeInplace(c_product, rlk);

        evaluator.multiplyMany(ciphers, rlkcpu, product);
        c_evaluator.multiplyMany(c_ciphers, rlk, c_product);

        Plaintext plain; PlaintextCuda c_plain;
        decryptor.decrypt(product, plain);
        c_decryptor.decrypt(c_product, c_plain);

        std::cout << plain.to_string() << std::endl;
        std::cout << c_plain.to_string() << std::endl;

        product = c_product.cpu();
        decryptor.decrypt(product, plain);
        std::cout << plain.to_string() << std::endl;
