...
namespace troy
{
    bool isMetadataValidFor(const Plaintext &in, const SEALContext &context, bool allow_pure_key_levels)
    {
        // Verify parameters
        ...
        if (in.isNttForm())
        {
            ...
            auto context_data_ptr = context.getContextData(in.parmsID());
            if (!context_data_ptr)
            {
                return false;
            }
            ...
            bool is_parms_pure_key = context_data_ptr->chainIndex() > context.firstContextData()->chainIndex();
            ...
            if (mul_safe(coeff_modulus.size(), poly_modulus_degree) != in.coeffCount())
            {
                return false;
            }
        }
        ...
        {
            auto &parms = context.firstContextData()->parms();
            size_t poly_modulus_degree = parms.polyModulusDegree();
            if (in.coeffCount() > poly_modulus_degree)
            {
                return false;
            }
        }
    ...
    bool isMetadataValidFor(const Ciphertext &in, const SEALContext &context, bool allow_pure_key_levels)
    {
        // Verify parameters
        if (!context.parametersSet())
        {
        ...
        auto &coeff_modulus = context_data_ptr->parms().coeffModulus();
        size_t poly_modulus_degree = context_data_ptr->parms().polyModulusDegree();
        if ((coeff_modulus.size() != in.coeffModulusSize()) || (poly_modulus_degree != in.polyModulusDegree()))
        {
            return false;
        }
        ...
        uint64_t correlationFactor = in.correctionFactor();
        uint64_t plain_modulus = context.firstContextData()->parms().plainModulus().value();
        if ((correlationFactor != 1 && (scheme == SchemeType::bfv || scheme == SchemeType::ckks)) ||
            ((correlationFactor == 0 || correlationFactor >= plain_modulus) && scheme == SchemeType::bgv))
        {
            return false;
        }
    ...
    bool isMetadataValidFor(const SecretKey &in, const SEALContext &context)
    {
        ...
        auto key_parms_id = context.keyParmsID();
        return isMetadataValidFor(in.data(), context, true) && (in.parmsID() == key_parms_id);
    }
    ...
    bool isMetadataValidFor(const PublicKey &in, const SEALContext &context)
    {
        ...
        auto key_parms_id = context.keyParmsID();
        return isMetadataValidFor(in.data(), context, true) && in.data().isNttForm() &&
               (in.parmsID() == key_parms_id) && (in.data().size() == SEAL_CIPHERTEXT_SIZE_MIN);
    }
    ...
    bool isMetadataValidFor(const KSwitchKeys &in, const SEALContext &context)
    {
        ...
        if (in.parmsID() != context.keyParmsID())
        {
            return false;
        }

        size_t decomp_mod_count = context.firstContextData()->parms().coeffModulus().size();
        for (auto &a : in.data())
        {
    ...
    bool isMetadataValidFor(const RelinKeys &in, const SEALContext &context)
    {
        ...
        return isMetadataValidFor(static_cast<const KSwitchKeys &>(in), context) && size_check;
    }

    bool isMetadataValidFor(const GaloisKeys &in, const SEALContext &context)
    {
        // Check the metadata; then we know context is OK
        bool metadata_check = isMetadataValidFor(static_cast<const KSwitchKeys &>(in), context);
        bool size_check = !in.size() || in.size() <= context.keyContextData()->parms().polyModulusDegree();
        return metadata_check && size_check;
    }

    bool isBufferValid(const Plaintext &in)
    {
        if (in.coeffCount() != in.dynArray().size())
        {
            return false;
        }
    ...
    {
        return isBufferValid(static_cast<const KSwitchKeys &>(in));
    }
    ...
    bool isDataValidFor(const Plaintext &in, const SEALContext &context)
    {
        ...
        {
            auto &parms = context.firstContextData()->parms();
            uint64_t modulus = parms.plainModulus().value();
            const Plaintext::pt_coeff_type *ptr = in.data();
            auto size = in.coeffCount();
    ...
    bool isDataValidFor(const Ciphertext &in, const SEALContext &context)
    {
        ...
        // Check the data
        auto context_data_ptr = context.getContextData(in.parmsID());
        const auto &coeff_modulus = context_data_ptr->parms().coeffModulus();
        size_t coeff_modulus_size = coeff_modulus.size();
        ...
        for (size_t i = 0; i < size; i++)
        {
            ...
            {
                uint64_t modulus = coeff_modulus[j].value();
                auto poly_modulus_degree = in.polyModulusDegree();
                for (; poly_modulus_degree--; ptr++)
                {
                    if (*ptr >= modulus)
                    {
    ...
    bool isDataValidFor(const SecretKey &in, const SEALContext &context)
    {
        ...
        // Check the data
        auto context_data_ptr = context.keyContextData();
        auto &parms = context_data_ptr->parms();
        auto &coeff_modulus = parms.coeffModulus();
        size_t coeff_modulus_size = coeff_modulus.size();
    ...
    bool isDataValidFor(const PublicKey &in, const SEALContext &context)
    {
        ...
        for (size_t i = 0; i < size; i++)
        {
            ...
            {
                uint64_t modulus = coeff_modulus[j].value();
                auto poly_modulus_degree = in.data().polyModulusDegree();
                for (; poly_modulus_degree--; ptr++)
                {
                    if (*ptr >= modulus)
                    {
    ...
    {
        return isDataValidFor(static_cast<const KSwitchKeys &>(in), context);
    }
