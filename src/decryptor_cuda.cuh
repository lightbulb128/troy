...
namespace troy
{
    ...
    */
    ...
    public:
        ...
        */
        DecryptorCuda(const SEALContextCuda&context, const SecretKeyCuda &secret_key);
        ...
        */
        void decrypt(const CiphertextCuda &encrypted, PlaintextCuda &destination);
        ...
        */
        ...
        // int invariantNoiseBudget(const CiphertextCuda &encrypted);

    private:
        void bfvDecrypt(const CiphertextCuda &encrypted, PlaintextCuda &destination);

        void ckksDecrypt(const CiphertextCuda &encrypted, PlaintextCuda &destination);

        void bgvDecrypt(const CiphertextCuda &encrypted, PlaintextCuda &destination);

        DecryptorCuda(const DecryptorCuda &copy) = delete;

        DecryptorCuda(DecryptorCuda &&source) = delete;

        DecryptorCuda &operator=(const DecryptorCuda &assign) = delete;

        DecryptorCuda &operator=(DecryptorCuda &&assign) = delete;
        ...
        void dotProductCtSkArray(const CiphertextCuda &encrypted, util::DevicePointer<uint64_t> destination);

        SEALContextCuda context_;

        std::size_t secret_key_array_size_ = 0;

        util::DeviceArray<std::uint64_t> secret_key_array_;
